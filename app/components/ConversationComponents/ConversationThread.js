...
import * as Form from '../styled/Form.js';
import { NavSpan } from '../styled/Nav.js';
import * as Font from '../styled/Font.js';

import EditorReadOnly from './EditorReadOnly.js';
import Editor from './Editor.js';
import RunCode from './RunCode.js';

import { fetchCurrentConversation, updateConversation } from '../../redux/conversations/thunks.js';
import { createReply, deleteReply } from '../../redux/replies/thunks.js';
import draftBody from '../../redux/body/actions.js';
      ...
      <Form.Container>
      {
        <div>
          <Font.h2>{conversation.title}</Font.h2>
          {
            ...
            <Font.Label>Answered</Font.Label>
            : null
          }
          { 
            conversation.replies && conversation.replies.map((reply, idx) => {
              return (
                <div key={reply.id}>
                  ...
                  { user.id === reply.userId && idx !== 0 ? <Button secondary disabled={isLoading} onClick={(e) => handleDeleteReply(e, reply)}>Delete</Button> : null }
                  { 
                    !conversation.hasAnswer ?
